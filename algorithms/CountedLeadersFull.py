...
class CountedLeadersFull(BaseEstimator, ClusterMixin, TransformerMixin):
    ...
    def fit_predict(self, X, y=None):
        ...
        distances_matrix = distance.squareform(distance.pdist(X, 'euclidean'))

        if X.shape[0] > 0:
            # initialize arrays with the first sample
            self.leaders_indexes.append(0)
            ...
            for index in range(1, X.shape[0]):
                ...
                sample_leader_index = self._get_sample_leader_index(index, distances_matrix)
                if sample_leader_index == -1:
                    # the sample has no leader
                    # create a new leader with it
                    self.leaders_indexes.append(index)
    ...
    def _get_sample_leader_index(self, sample_index, distances_matrix):
        result = np.where(distances_matrix[sample_index][self.leaders_indexes] < self.threshold_distance)
        if len(result[0]) > 0:
            return result[0][0]
        else:
            return -1
